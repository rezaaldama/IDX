...
def fetch_and_transform_data(ticker_symbol, statement_type):
    try:
        ...
        print(f"An error occurred for {ticker_symbol} ({statement_type}): {e}")
    
    return None

# Function to fetch and combine data for all ticker symbols
def fetch_and_combine_data(ticker_symbols):
    combined_dataframes = []

    for ticker_symbol in ticker_symbols:
        # Fetch and transform data for each statement type
        data_frames = [
            fetch_and_transform_data(ticker_symbol, statement_type)
            for statement_type in ['CF', 'IS', 'BS']
        ]

        # Filter out None values and concatenate dataframes
        combined_data = pd.concat([df for df in data_frames if df is not None], ignore_index=True)
        combined_dataframes.append(combined_data)

    return combined_dataframes

combine_and_transform_data(ticker_symbols)